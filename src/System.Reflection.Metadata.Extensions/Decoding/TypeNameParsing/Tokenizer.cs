...
namespace System.Reflection.Metadata.Decoding
{
    ...
    internal partial class Tokenizer
    {
        ...
        public void Skip(TokenType expected)
        {
            ...
            if (token == null)
            {
                throw FormatException(TypeNameFormatErrorId.DelimiterExpected_EncounteredEndOfString, Strings2.TypeFormat_DelimiterExpected_EncounteredEndOfString, (char)expected);
            }

            if (token.TokenType != expected)
            {
                throw FormatException(TypeNameFormatErrorId.DelimiterExpected, Strings2.TypeFormat_DelimiterExpected, (char)expected, token.Value);
            }
        }
        ...
        public void Close()
        {
            Token token = ReadToken();
            if (token != null)
                throw FormatException(TypeNameFormatErrorId.EndOfStringExpected_EncounteredExtraCharacters, Strings2.TypeFormat_EndOfStringExpected_EncounteredExtraCharacters, token.Value);
        }
        ...
        private void CheckIdentifier(IdentifierOptions options, string identifier)
        {
            ...
            if (token == null)
            {
                throw FormatException(TypeNameFormatErrorId.IdExpected_EncounteredEndOfString, Strings2.TypeFormat_IdExpected_EncounteredEndOfString);
            }

            // Otherwise, we must have hit a delimiter as whitespace will have been consumed as part of the identifier
            throw FormatException(TypeNameFormatErrorId.IdExpected_EncounteredDelimiter, Strings2.TypeFormat_IdExpected_EncounteredDelimiter, token.Value);
        }

        private void CheckIdentifierAfterTrim(IdentifierOptions options, string identifier)
        {
            if (!IsValidIdentifier(options, identifier))
                throw FormatException(TypeNameFormatErrorId.IdExpected_EncounteredOnlyWhiteSpace, Strings2.TypeFormat_IdExpected_EncounteredWhiteSpace);
        }
        ...
        private Token HandleEscapeSequence(Token token, StringReader reader)
        {
            ...
            if (token == null)
                throw FormatException(reader, TypeNameFormatErrorId.EscapedDelimiterExpected_EncounteredEndOfString, Strings2.TypeFormat_EscapedDelimiterExpected_EncounteredEndOfString);

            // Next delimiter is always considered a literal
            if (!token.IsDelimiter && !token.IsEscapeSequence)
                throw FormatException(reader, TypeNameFormatErrorId.EscapedDelimiterExpected, Strings2.TypeFormat_EscapedDelimiterExpected, token.Value);
        ...
        private static FormatException FormatException(StringReader reader, TypeNameFormatErrorId errorId, string format, params object[] arguments)
        {
            ...
            string positionMessage = String.Format(CultureInfo.CurrentCulture, Strings2.TypeFormat_Position, reader.Position);
